...
<!-- Favicon uses the emerald brand color matching the desktop app's
     --accent-primary (#10b981), not the landing page's indigo. -->
...
/* Theme: "Warm Structured Glass" mirrored from the desktop app's
   src/renderer/styles/globals.css. Emerald brand (#10b981), warm-graphite
   base (#0f1012), warm-rose negative (#fb7185), amber highlight (#f59e0b),
   and 6px radii — NOT the landing page's indigo+2px identity. Marketing
   stays on the landing page subdomain; the app surface is its own brand. */
:root{
  --bg:#0f1012;--bg-card:#16171a;--bg-elevated:#1d1e22;
  --border:rgba(255,255,255,0.06);--border-strong:rgba(255,255,255,0.11);
  --text:#ECEAE4;--text-dim:#A8A39A;--text-muted:#6B665D;--text-bright:#ffffff;
  --accent:#10b981;--accent-hover:#34d399;
  --green:#34d399;--blue:#60a5fa;--purple:#c084fc;--red:#fb7185;--amber:#f59e0b;
  --radius:6px;
}
...
/* NAV — emerald accent underline + warm-graphite glass surface */
nav.shell-nav{position:fixed;top:0;left:0;right:0;z-index:100;background:rgba(15,16,18,0.82);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);padding:0 1.5rem;height:60px;display:flex;align-items:center;justify-content:space-between}
...
.btn-nav{background:var(--accent);color:#fff!important;padding:7px 16px;border-radius:var(--radius);font-weight:600;font-size:0.8rem;transition:background 0.2s}
...
.btn{background:var(--accent);color:#fff;padding:10px 20px;border-radius:var(--radius);font-weight:600;font-size:0.85rem;border:none;cursor:pointer;transition:background 0.2s;display:inline-flex;align-items:center;gap:8px}
...
label.field input,label.field select,label.field textarea{background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);padding:10px 12px;color:var(--text-bright);font-size:0.9rem;text-transform:none;letter-spacing:normal;font-weight:400;transition:border-color 0.15s}
...
table.data{width:100%;border-collapse:collapse;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden}
...
.badge{display:inline-block;padding:3px 9px;border-radius:var(--radius);font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;background:var(--bg-elevated);color:var(--text-dim)}
...
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);padding:12px 20px;font-size:0.85rem;color:var(--text-bright);box-shadow:0 10px 30px rgba(0,0,0,0.4);opacity:0;transition:opacity 0.2s;pointer-events:none;z-index:1000}
...
.error-banner{background:rgba(239,68,68,0.1);border:1px solid var(--red);color:#fca5a5;padding:12px 16px;border-radius:var(--radius);margin-bottom:1rem;font-size:0.85rem}
