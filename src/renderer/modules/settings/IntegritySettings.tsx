...
const IntegritySettings: React.FC = () => {
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 900 }}>
      ...
      {result && (
        <div className="block-card" style={{ padding: 16, borderLeft: '3px solid ' + (result.ok ? 'var(--color-accent-income)' : 'var(--color-accent-warning)') }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            {result.ok ? (
              <ShieldCheck size={16} style={{ color: 'var(--color-accent-income)' }} />
            ) : (
          ...
          {result.schemaDrift.missingFromExemption.length > 0 && (
            <div style={{ marginTop: 8, fontSize: 12 }}>
              <div style={{ fontWeight: 700, color: 'var(--color-accent-warning)' }}>
          ...
          {Object.keys(result.orphans).length > 0 && (
            <div style={{ marginTop: 12, fontSize: 12 }}>
              <div style={{ fontWeight: 700, color: 'var(--color-accent-warning)', marginBottom: 4 }}>
              ...
              {Object.entries(result.orphans).map(([target, info]) => (
                <div key={target} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px dashed var(--color-border-primary)' }}>
                  ...
                  <button
                    ...
                    style={{ color: 'var(--color-accent-warning)', borderColor: 'var(--color-accent-warning)' }}
                  >
                    <Wrench size={11} />
