...
function getHealthBadge(outstanding: number, totalInvoiced: number, avgPaymentDays: number): { label: string; color: string; bg: string } {
  ...
  if (outstandingRatio > 0.5 || avgPaymentDays > 60) return { label: 'At Risk', color: 'var(--color-accent-expense)', bg: 'var(--color-accent-expense-bg)' };
  if (outstandingRatio > 0.2 || avgPaymentDays > 30) return { label: 'Watch', color: 'var(--color-accent-warning)', bg: 'var(--color-accent-warning-bg)' };
  return { label: 'Healthy', color: 'var(--color-accent-income)', bg: 'var(--color-accent-income-bg)' };
}
