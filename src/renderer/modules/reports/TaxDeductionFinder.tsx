...
const TYPE_ICON: Record<string, React.ReactNode> = {
  ...
  documentation: <Info size={14} style={{ color: 'var(--color-text-muted)' }} />,
};
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {data.suggestions.map((s, i) => (
                <div key={i} className="block-card" style={{
                  padding: 16,
                  borderLeft: '3px solid ' + (s.confidence === 'high' ? 'var(--color-accent-income)' : s.confidence === 'medium' ? 'var(--color-accent-warning)' : 'var(--color-text-muted)'),
                }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--color-text-muted)', display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                    ...
                    <span>Confidence: <strong style={{ color: s.confidence === 'high' ? 'var(--color-accent-income)' : s.confidence === 'medium' ? 'var(--color-accent-warning)' : 'var(--color-text-muted)' }}>{s.confidence}</strong></span>
