          ...
          <td className="text-text-primary font-medium">
            {isEditingDesc ? (
              ...
              <div className="flex items-center gap-1.5" onDoubleClick={(e) => { e.stopPropagation(); startEdit(exp.id, 'description', exp.description); }}>
                ...
                {matchHint ? <span title="A bank transaction matches this expense" style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 'var(--app-radius)', background: 'var(--color-accent-blue-bg)', color: 'var(--color-accent-blue)' }}><Banknote size={10} style={{ display: 'inline', marginRight: 2 }} />MATCH?</span> : null}
                ...
                {Number((exp as any).allocation_count) > 0 ? <span title={`Split across ${(exp as any).allocation_count} allocation target${Number((exp as any).allocation_count) !== 1 ? 's' : ''}`} style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 'var(--app-radius)', background: 'var(--color-accent-blue-bg)', color: 'var(--color-accent-blue)' }}>SPLIT×{(exp as any).allocation_count}</span> : null}
  ...
  return (
    <div className="space-y-4" style={{ paddingBottom: someSelected ? '80px' : undefined }}>
      ...
      {unmatchedRefunds.length > 0 && (
        <div className="block-card p-3 flex items-center gap-3" style={{ borderLeft: '3px solid var(--color-accent-purple)' }}>
