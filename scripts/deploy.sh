...
ssh -i "$SSH_KEY" "$VPS" "cd /opt/bap-server && npm run build && (pm2 restart bap-server --update-env 2>/dev/null || pm2 start dist/index.js --name bap-server --cwd /opt/bap-server --restart-delay 3000 --max-restarts 5) && pm2 save"
