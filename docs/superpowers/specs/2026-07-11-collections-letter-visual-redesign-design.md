...
- Add `table-layout: fixed;` plus an explicit `<colgroup>` sized to each table's actual columns (the summary table's 8 columns become: `#`, Item, Serial, Model, Condition, Value, Date Issued, Status — Status split from Date, each getting its own column instead of a combined `"Stolen · March 4, 2026"` string).
